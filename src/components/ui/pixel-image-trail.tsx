...
interface PixelImageTrailProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  /** When true, show the full source image instead of only the trail. */
  fullyRevealed?: boolean
  /** Fraction of the contained fit size used for the centered logo (0–1). */
  imageScale?: number
}
...
export function PixelImageTrail({
  ...
  const clampedImageScale = Math.min(1, Math.max(0.1, imageScale))
  ...
  React.useEffect(() => {
    ...
    const drawPixel = (pixel: TrailPixel, opacity: number) => {
      ...
      const renderScale = fitScale * clampedImageScale
      const renderedWidth = image.naturalWidth * renderScale
      const renderedHeight = image.naturalHeight * renderScale
      const offsetX = (width - renderedWidth) / 2
      const offsetY = (height - renderedHeight) / 2

      const sourceX = (x - offsetX) / renderScale
      const sourceY = (y - offsetY) / renderScale
      const sourceSize = size / renderScale

      if (
        sourceX + sourceSize <= 0 ||
        sourceY + sourceSize <= 0 ||
  ...
  return (
    ...
    >
      <img
        ...
        alt={alt}
        className={cn(
          "pointer-events-none absolute top-1/2 left-1/2 h-auto w-auto -translate-x-1/2 -translate-y-1/2 object-contain",
          fullyRevealed ? "opacity-100" : "opacity-0"
        )}
        style={{
          maxWidth: `${clampedImageScale * 100}%`,
          maxHeight: `${clampedImageScale * 100}%`,
        }}
        draggable={false}
      />
      <canvas
